...
def construct_agent(oftype: str, player: int, configs: dict):
    ...
    if oftype == "human":
        ...
        agent_func = lambda x: mcts.best_move(
            x, player, mcts_timelim, score0, score1, mcts_c_param
        )

    elif oftype == "minimax":
        mm_depth = configs["minimax"]["search_depth"]
        mm_randn = configs["minimax"]["randomness"]
        agent_func = lambda x: minimax.best_move(x, player, mm_depth, mm_randn, score0, score1)
...
def main_graphical_loop(game_inst, win_inst, agent1_func, agent2_func):
    ...
    while not game_inst.game_over and win_inst.open:
        ...
        score0.append(min_agent.board_score(game_inst.board, 0))
        score1.append(min_agent.board_score(game_inst.board, 1))
    print(score0)
    print(score1)
    x =[]
    c=0
    for i in range(len(score0)):
        x.append(c)
        c=c+1

    plt.clf()
    plt.plot(x, score0, label='mcts', marker='o', color='red')
    plt.plot(x, score1, label='minimax', marker='x', color='green')
    plt.xlabel('game iterations')
    plt.ylabel('scores of algos')
    plt.title('mini vs mcts t=30 and d=3')
    plt.legend()
    plt.savefig("mini_mcts_t=30_d=3(5x5).png")
